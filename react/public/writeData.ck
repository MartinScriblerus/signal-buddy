                    ...
                    fun void playSixteenthNote( int count, int numeratorSignature, string file, dur beat, dur bar, int pat_test[][] ) {
                        ...
                        if ((playForce == 1 | ((count + 1) % pat_test[0][0] == 0) && (pat_test[0][1] == 0 | (pat_test[0][1] != 0 && (count + 1) % pat_test[0][1] != 0))) & (doNotPlay != 1) & (file != "")) {
                    ...
                    if (secLenBeat > .000000001) {
                        ${lastBpm} => int numeratorSignature;
                        secLenBeat::second => dur beat;
                        (secLenBeat * numeratorSignature)::second => dur bar;
                        "" => string file;
                        "${createdFilesList}" => file;
               
                        if (file != "") {
                            file => string type;
                            <<< "GOT FILE" >>>;
                       
                            type => string hatFilename;
                
                            type => string snareFilename;
                            type => string kickFilename;
                            <<< "ERRRR" >>>;
                            <<< type >>>;
                        [ type ] @=> string fileArray[];
...
// float secLenBeat;
// <<< "TRACK NAMES ", me.arg(4) >>>;
// me.arg(4) => string trackName;
...
//     (secLenBeat * numeratorSignature)::second => dur bar;
//     // <<< "HERE!!! ", me.arg(4)[0] >>>;
//     '/' + trackName => string hatFilename;
//     // me.dir() + "MinipopsHH.ck" => string hatFilename;
//     '/' + trackName => string snareFilename;
//     '/' + trackName => string kickFilename;
